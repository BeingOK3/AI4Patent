...
Before `start` or `run`, require the caller to provide `--model-base-url`, `--model`, and the model API Key through `DEEPSEEK_API_KEY` (or a specifically named environment variable selected with `--api-key-env`). Never accept a literal key on the command line, print it, copy it into a report, or write it to a file. The CLI forwards all three runtime values to the Run creation request, and the backend holds the API Key only in process memory for that Run.
...
python scripts/idea_workflow.py start --model-base-url https://api.deepseek.com --model deepseek-v4-flash --case-title "Token heat cache" --idea-file /tmp/idea.txt --mode standard --candidate-max 80 --deep-min 10 --deep-max 20
