...
FIXTURE = Path("backend/tests/fixtures/google_patents_search.html").read_bytes()
...
class GooglePatentsSearchTests(unittest.TestCase):
    ...
    def test_url_encodes_query_limit_country_and_page(self) -> None:
        ...
        self.assertIn("q=cache+affinity+scheduling", url)
        self.assertIn("num=25", url)
        self.assertIn("page=2", url)
        self.assertIn("country=US%2CEP", url)
